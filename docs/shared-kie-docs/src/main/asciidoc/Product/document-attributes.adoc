...
:MAVEN_ARTIFACT_VERSION: 7.5.0.Final-redhat-4
...
//Delete INSTALLING_DM_ON_PREMISE attr once all refs to it are replaced with the appropriate installation link.
...
//Delete DM_ON_OPENSHIFT attr once the other deploying on OS docs are published. Replace cross-refs as needed.
:DEPLOYING_AUTHORING_ON_OPENSHIFT: Deploying a {PRODUCT} {PRODUCT_VERSION} authoring environment on Red Hat OpenShift Container Platform
:DEPLOYING_IMMUTABLE_ON_OPENSHIFT: Deploying a {PRODUCT} {PRODUCT_VERSION} immutable server environment on Red Hat OpenShift Container Platform
:DEPLOYING_MANAGED_ON_OPENSHIFT: Deploying a {PRODUCT} {PRODUCT_VERSION} managed server environment on Red Hat OpenShift Container Platform

:RUNNING_ROSTER_FOR_PLANNER: Running the employee roster starter application for {PLANNER}
...
:URL_RUNNING_ROSTER_FOR_PLANNER: {URL_BASE_ENTERPRISE}/running_the_employee_roster_starter_application_for_red_hat_business_optimizer
