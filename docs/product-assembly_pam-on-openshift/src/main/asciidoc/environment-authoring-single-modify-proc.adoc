...
. Replace the block named `H2 database parameters` with the block named `External database parameters`. (Take this block and all subsequent replacement blocks from the `rhpam70-kieserver-externaldb.yaml` file.)
