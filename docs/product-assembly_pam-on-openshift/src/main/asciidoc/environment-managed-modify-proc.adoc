...
. Replace the block named `MySQL database parameters` with the block named `PosgrreSQL database parameters`. (Take this block and all subsequent replacement blocks from the `rhpam70-kieserver-postgresql.yaml` file.)
+
...
* If you want to use an external database server, make the following changes for every replicated {KIE_SERVER} pod number, for example, `1` and `2` in the unmodified template. `N` refers to the pod number, for example, `1`. 
:
