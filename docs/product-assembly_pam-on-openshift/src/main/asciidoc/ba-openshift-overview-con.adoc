...
+
A database backend is normally required for {KIE_SERVER}. You can provide a database server in another OpenShift pod or configure an execution server on OpenShift to use any other database server. Alternatively, {KIE_SERVER} can use an in-memory h2 database; in this case, the database is not persistent (possibly leading to wrong results) and the pod can not be scaled.
+
