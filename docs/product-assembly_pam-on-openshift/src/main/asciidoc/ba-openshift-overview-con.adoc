...
ifeval::["{context}"=="openshift-ansible-playbook"]
You can use the {PRODUCT} Ansible Playbook to deploy a {PRODUCT} environment on OpenShift in an interactive process. You can set all possible configuration values during this process. The installation process can generate all the required secrets automatically.
endif::[]
