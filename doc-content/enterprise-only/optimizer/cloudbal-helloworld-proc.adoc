...
.Procedure
. Download and configure the examples in your preferred IDE. For instructions about downloading and confogiring examples in an IDE, see <<optimizer-running-the-examples-in-an-IDE-proc>>.
...
+ 
Building the `Solver` is the most complicated part of this procedure. For more detail, see <<cloudbal-solver-configuraton-ref>>.
+
[source,java,options="nowrap"]
----
        SolverFactory<CloudBalance> solverFactory = SolverFactory.createFromXmlResource(
