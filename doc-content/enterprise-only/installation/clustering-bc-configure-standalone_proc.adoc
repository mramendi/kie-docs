...
. Edit or add the following properties under the `<system-properties>` element and replace the following placeholders:
...
----
+
. If the {DATAGRID} deployment requires authencication, also edit or add the following properties under the `<system-properties>` element and replace the following placeholders:
