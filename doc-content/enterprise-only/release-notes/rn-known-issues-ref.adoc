...
.Validation for a signal name fails if the name includes spaces or colons (`:`) [https://issues.redhat.com/browse/RHPAM-2557[RHPAM-2557]]
...
Actual result: The spaces do not appear and the field contains `Milestone2:Ordershipped`. After you press *Tab* an *Invalid characters* error message appears.

Workaround: Use signal names that contain only Latin letters (`A...Z`, `a...z`), numbers (`0...9`), and the underscore character (`_`).

[NOTE]
====
If you import or migrate a project to the new process designer and any signal name in the project contains spaces or special characters, a warning displays. You can ignore the warning as the execution of the project is not affected.
====
