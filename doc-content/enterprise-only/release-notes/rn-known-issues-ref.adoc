...
.The `offliner` tool reports errors when downloading artifacts for an offline Maven repository [https://issues.redhat.com/browse/RHPAM-2234[RHPAM-2234]]

Issue: When you run the `offliner` tool to download artifacts for an offline Maven repository, the tool reports that downloading some artifacts has failed. The error message is `The artifact was not found in any of the provided repositories`.
...
In other cases, for example, for `maven-compiler-plugin` or `docbook` artifacts, specify one more source repository and run the `offliner` command again:
