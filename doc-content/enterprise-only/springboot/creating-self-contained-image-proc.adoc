...
ifdef::PAM[]
+
. Optional: if you want to be able to configure the {KIE_SERVER} in the JAR file to communicate with a {CENTRAL} Monitoring instance using WebSockets, add the following lines to the `pom.xml` file under the `<dependencies>` tag:
+
[source, xml]
----
<dependency>
