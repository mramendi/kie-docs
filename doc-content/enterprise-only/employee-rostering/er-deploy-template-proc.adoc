...
.Procedure
. Download the `{PRODUCT_FILE}-openshift-templates.zip` file from the {PRODUCT_DOWNLOAD_LINK}[Software Downloads] page for {PRODUCT} {PRODUCT_VERSION}.
. Unzip the downloaded archive. Locate the `{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-optaweb-employee-rostering-trial-ephemeral.yaml` file.
