...
. When all the data is available, modify the rules. For most modifications, you need to add a new rule. The rules are located in the `src/main/resources/org/optaweb/employeerostering/server/solver/employeeRosteringScoreRules.drl` file of the `employee-rostering-server` module. 
+
