...
[IMPORTANT]
====
To export built KJAR files from your projects in {CENTRAL} to the external Maven repository, you must also modify the `settings.xml` file in every project. For instructions about modifying the `settings.xml` file for this purpose, see {URL_DESIGNING_BUSINESS_PROCESSES}#export-project-to-maven[_{DESIGNING_BUSINESS_PROCESSES}_].
====
endif::[]
