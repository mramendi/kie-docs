...
ifdef::PAM[`{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-image-streams.yaml`]
file.
...
ifdef::PAM[]
download the `{PRODUCT_FILE}-openshift-templates.zip` product deliverable file from the https://access.redhat.com/jbossnetwork/restricted/listSoftware.html[Software Downloads] page for {PRODUCT} {PRODUCT_VERSION}. Extract the `{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-image-streams.yaml` file from it.
endif::PAM[]
