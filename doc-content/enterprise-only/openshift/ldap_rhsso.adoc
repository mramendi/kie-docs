...
*** `KIE_ADMIN_USER`: default name `adminUser`, roles: `kie-server,rest-all,admin,kiemgmt,Administrators`
ifdef::kieandcentral[]
*** `KIE_SERVER_CONTROLLER_USER`: default name `controllerUser`, roles: `kie-server,rest-all,guest`
*** `{CENTRAL_CAPITAL_UNDER}_MAVEN_USERNAME` (not needed if you configure the use of an external Maven repository): default name `mavenUser`. No roles are required.
endif::kieandcentral[]
ifdef::centralonly[]
...
ifndef::centralonly[]
*** `KIE_SERVER_USER`: default name `executionUser`, roles `kie-server,rest-all,guest`
endif::centralonly[]
+
