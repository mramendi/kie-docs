...
.. In the RH-SSO or LDAP service, create all user names in the deployment parameters. If you do not set any of the parameters, create the default user names:
*** `KIE_ADMIN_USER`: `adminUser`
ifdef::kieandcentral[]
*** `KIE_SERVER_CONTROLLER_USER`: `controllerUser`
*** `{CENTRAL_CAPITAL_UNDER}_MAVEN_USERNAME`: `mavenUser` (unless you have configured the use of an external Maven repository)
endif::kieandcentral[]
ifdef::centralonly[]
*** `KIE_SERVER_MONITOR_USER`: `monitorUser`. You *must not* change this user name. You also *must* configure the `KIE_SERVER_MONITOR_PASSWORD` parameter to the same value as the password for this user in the RH-SSO service. Otherwise, the suggested parameter settings for the server deployments will be incorrect. 
endif::centralonly[]
ifndef::centralonly[]
...
*** *RH-SSO Disable SSL Certificate Validation* (`SSO_DISABLE_SSL_CERTIFICATE_VALIDATION`): Set to `true` if your RH-SSO installation does not use a valid HTTPS certificate. 
.. To configure LDAP, set the `LDAP*` parameters of the template. These parameters correspond to the settings of the LdatExtended Login module of {EAP}. For instructions about using these settings, see https://access.redhat.com/documentation/en-us/red_hat_jboss_enterprise_application_platform/7.0/html-single/login_module_reference/#ldapextended_login_module[LdapExtended Login Module]. 
+
