...
= Migrating information from a deployment on {OPENSHIFT} version 3
...
Before migrating information, you must deploy a new {PRODUCT} infrastructure on {OPENSHIFT} version 4 using the operator. Include the same elements in this infrastructure as those present in the old deployment. For example:
...
* For any existing {KIE_SERVER} that uses an external database server, deploy a new {KIE_SERVER} that uses the same external database server with the same credentials.
endif::PAM[]
