...
** *KIE Server Mode* (`KIE_SERVER_MODE`): Set to `DEVELOPMENT` if you need to deploy a `SNAPSHOT` version of a KJAR artifact. The default value is `PRODUCTION`, which does not allow deployment of `SNAPSHOT` versions.
