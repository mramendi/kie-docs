...
from the `{PRODUCT_FILE}-openshift-templates.zip` product deliverable file. You can download the file from the {PRODUCT_DOWNLOAD_LINK}[Software Downloads] page.

When you deploy an immutable {KIE_SERVER}, the deployment procedure retrieves the KJAR files for any services that must run on this server and includes them in the server image.
...
* In the OpenShift Web UI, select *Add to Project -> Import YAML / JSON* and then select or paste the templatee file.
