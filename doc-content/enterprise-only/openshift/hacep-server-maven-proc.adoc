...
You can implement the HA CEP server with a separate Maven repository and place your KJAR service in the repository. In this case, you can update the KJAR service at any time by updating it in the Maven repository and then updating the group, artifact, and version (GAV) values in an environment variable of the server pod.
...
. Replace `<id_iser>` with the user ID that you entered in the Docker file in a previous step.
...
----
+
. To update the KJAR service at any later time, complete the following steps:
.. Upload the new KJAR service to the Maven repository.
.. Log in to the project with the `oc` command.
.. Use the `oc get pods` command to detect the name of the pod in which the HA CEP server is running.
.. Enter the following command:
+
----
oc set env pod/<podname> KJARGAV=<gav>
----
+
In this command, replace `<podname>` with the name of the pod and `<gav>` with the GAV of the new KJAR service.
