...
You can deploy fixed a managed server environment that, in a single deployment, includes several different pods running {KIE_SERVER}. No processes are initially loaded on the servers. The database servers are, by default, also run in pods. Each {KIE_SERVER} pod can be separately scaled as necessary.
...
You can not add or remove {KIE_SERVERS} at a later time.
