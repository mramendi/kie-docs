...
To start deploying a freeform environment, deploy {CENTRAL} Monitoring and a single managed {KIE_SERVER}, which uses a PostgreSQL database server in a pod. No services are loaded on the deployed {KIE_SERVER}. Use {LOCAL_CENTRAL} to deploy and undeploy services on the server.
