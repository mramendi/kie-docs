...
. If you want to update the KJAR version in an implementation that uses the method described in <<hacep-server-maven-proc>>, add a `UpdateKJarGAV` call to the client, similar to the following code:
+
[source,java]
----
        ...
        try (RemoteStreamingKieSession producer = RemoteStreamingKieSession.create(props, envConfig)){
            producer.updateKJarGAV("org.kie:fake-jar:0.1-SNAPSHOT");
        }
----
+
You must ensure that a KJAR with the specified GAV is available in the Maven repository when this call is executed.
+
