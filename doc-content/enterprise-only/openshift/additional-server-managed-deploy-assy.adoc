...
If you deployed a single authoring environment using the `{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-authoring.yaml` template, in order for the {LOCAL_CENTRAL} to connect to the {KIE_SERVER} you must enable the `OpenShiftStartupStrategy` setting for your environment. For instructions about enabling the `OpenShiftStartupStrategy` setting, see <<startupstrategy-enable-proc>>. You do not need to complete this procedure for a high-availability authoring environment.
