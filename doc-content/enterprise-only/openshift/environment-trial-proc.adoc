...
This environment is intended for test and demonstration access. For this purpose, it supports cross-origin resource sharing (CORS). This means that {KIE_SERVER} endpoints can be accessed using a browser, with other resources on the page being provided by other servers. While {KIE_SERVER} endpoints are normally intended for REST calls, this feature is needed in some demonstration configurations. 
