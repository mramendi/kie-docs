...
.. *Java max initial memory*: The maximum amount of memory that can be initially used for the Java Virtual Machine for {CENTRAL}. If the initial allocated memory, as set in the *Java initial memory ratio* parameter, would otherwise be greater than this value, the amount of memory set in this value is allocated. The default value is `4096Mb`.
