...
* `{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-sit.yaml` if you need to use minimal resources and do not need high-availability configuration for {LOCAL_CENTRAL} and Smart Router
* `{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-prod.yaml` if you need high-availability configuration for {LOCAL_CENTRAL} and Smart Router
+
You can extract the template files from the `{PRODUCT_FILE}-openshift-templates.zip` product deliverable file. You can download the file from the https://access.redhat.com/jbossnetwork/restricted/listSoftware.html[Software Downloads] page for {PRODUCT} {PRODUCT_VERSION}.
...
----
oc new-app -f <template-file-name> -p {CENTRAL_CAPITAL_UNDER}_HTTPS_SECRET={CENTRAL_ONEWORD}-app-secret -p KIE_SERVER_HTTPS_SECRET=kieserver-app-secret -p MAVEN_REPO_URL=<maven-repo-url>
----
+
...
** *Business Central Server Keystore Secret Name* (`{CENTRAL_CAPITAL_UNDER}_HTTPS_SECRET`): The name of the secret for {CENTRAL}, as created in <<secrets-central-create-proc>>.
...
** *Business Central Server Certificate Name* (`{CENTRAL_CAPITAL_UNDER}_HTTPS_NAME`): The name of the certificate in the keystore that you created in <<secrets-central-create-proc>>.
** *Business Central Server Keystore Password* (`{CENTRAL_CAPITAL_UNDER}_HTTPS_PASSWORD`): The password for the keystore that you created in <<secrets-central-create-proc>>.
