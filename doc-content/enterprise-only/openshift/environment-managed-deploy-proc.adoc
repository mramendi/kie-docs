...
** *KIE Server Mode* (`KIE_SERVER_MODE`): The default value is `PRODUCTION`; in this mode, you can not deploy `SNAPSHOT` versions of KJAR artifacts and can not change versions of an artifact in an existing container. (You can still create a new container on the same {KIE_SERVER} to deploy a new version). If you want to deploy `SNAPSHOT` versions or to change versions of an artifact in an existing container, set this parameter to `DEVELOPMENT`. 
