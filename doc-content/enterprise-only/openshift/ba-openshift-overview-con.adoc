...
+
You can deploy two types of managed environment. In a _freeform_ server environment, you initially deploy {CENTRAL} Monitoring and one {KIE_SERVER}. You can additionally deploy any number of {KIE_SERVERS}. {CENTRAL} Monitoring can connects to all servers in the same namespace. 
+
