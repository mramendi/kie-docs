...
* If you want to configure the immutable {KIE_SERVER} to receive and respond to API messages using JMS, communicating with an AMQ server, use the `{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-prod-immutable-kieserver-amq.yaml` template file.
