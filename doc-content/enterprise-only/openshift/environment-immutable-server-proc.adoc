...
// definitions for RH-SSO include, though the include itself is not used for now as RH-SSO is not in the template
:kieonly:
...
ifdef::PAM[]
To deploy an immutable {KIE_SERVER}, use the `{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-prod-immutable-kieserver.yaml` template file. You can extract this file 
endif::PAM[]
ifdef::DM[]
You can deploy an immutable {KIE_SERVER} with or without support for HTTPS. 

To deploy an immutable {KIE_SERVER} with support for HTTPS, use the `{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-kieserver-https-s2i.yaml` template file. To deploy an immutable {KIE_SERVER} without support for HTTPS, use the `{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-kieserver-basic-s2i.yaml` template file. You can extract these files
endif::DM[]
from the `{PRODUCT_FILE}-openshift-templates.zip` product deliverable file. You can download the file from the https://access.redhat.com/jbossnetwork/restricted/listSoftware.html[Software Downloads] page for {PRODUCT} {PRODUCT_VERSION}.
...
* In the OpenShift Web UI, select *Add to Project -> Import YAML / JSON* and then select or paste the ifdef::PAM[`{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-prod-immutable-kieserver.yaml`
ifdef::DM[`{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-kieserver-https-s2i.yaml` or `{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-kieserver-basic-s2i.yaml`]
file. 
...
[subs="verbatim,macros"]
----
oc new-app -f <template-path>/{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-prod-kieserver-https-s2i.yaml -p <parameter>=<value> ...
----
+
or
+
----
oc new-app -f <template-path>/{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-prod-kieserver-basic-s2i.yaml -p <parameter>=<value> ...
----
endif::DM[]
+
