...
By default, the tempplaye for deploying an immutable server using S2I creates a separate PostgreSQL pod to provide the database server for each replicable {KIE_SERVER}. If you prefer to use MySQL or to use an external server (outside the OpenShift project), you must modify the `{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-prod-immutable-kieserver.yaml` template before deploying the server.
