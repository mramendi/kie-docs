...
* *Red Hat - Single Sign-On* or *LDAP* configured and  *Process server*>**Sets of Process Servers** exceeding `1`.
====
+
. Click *Next* to commence deployment.
+
After deploying the environment, you can access it using the HTTPS routes displayed in the OpenShift Web console. HTTP routes are not provided.
