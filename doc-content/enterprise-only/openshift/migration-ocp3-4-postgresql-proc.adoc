...
----
+
. Create a dump of the database using the recorded username and database name, for example:
+
[subs="attributes,verbatim,macros,specialchars,replacements"]
----
...
----
+
. Restore the database dump using the recorded username and database name, for example:
+
[subs="attributes,verbatim,macros,specialchars,replacements"]
----
...
+
Ignore database error messages. These messages are displayed because a database with the sane name already exists.
+ 
. Stop the port forwarding in the separate window using the `Ctrl`+`C` key combination.
