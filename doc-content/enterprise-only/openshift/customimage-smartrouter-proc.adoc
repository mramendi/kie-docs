...
By default, Smart Router routes requests based on the container alias. If severak {KIE_SERVERS} provide a servcie with teh same container alias, Smart Router balances the load between them.
...
+
. Create a work directory for creating the custom image, copy the generated JAR file into the directory, and then change to the directory, for example:
+
[subs="attributes,verbatim,macros,quotes"]
----
...
+
. Create a `Dockerfile` file that defines a custom image based on the base image. The file must add the JAR file and the new `openshift-launch.sh` file, change the current user to `root`, set the necessary permissions for the `openshift-launch.sh` file, and then revert to `USER 185`, the {EAP} user. The following example shows the content of the `Dockerfile` file:
+
[subs="attributes,verbatim,macros,quotes"]
----
...
* *Image*: The custom image name, for example, {PRODUCT_INIT}-{componentimage}-rhel8-custom
