...
. Under *Admin user*, enter the user name and password for the administrative user for {PRODUCT} in the *Username* and *Password* fields. If you use RH-SSO or LDAP authentication, the same user must be configured in your auctentication system with the `kie-server,rest-all,admin` roles for {PRODUCT}.
...
If you want to deploy the environment with the default configuration, click *Finish*, then click *Deploy* to deploy the environment. Otherwise, continue to set other configuration parameters.
