[id='template-deploy-prepare-{subcontext}-proc']
...
:template_name: {PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-prod-immitable-kieserver.yaml
...
* In the OpenShift Web UI, select *Add to Project -> Import YAML / JSON* and then select or paste the `{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-prod-managed.yaml` file. In the *Add Template* window, ensure *Process the template* is selected and click *Continue*.
