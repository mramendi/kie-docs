...
--
+
. Optional: If you want to use client roles, also add the following system property:
+
--
[source,xml,subs="attributes+"]
----
...
By default, the client resource name is `kie`. If you want to use a custom client resource name, add the following system property:
...
* `resource` is the name for the client that you created. If you want to use client roles with a different client resource name, replace `kie` with the client resource name, for example, `<resource>customClient</resource>`.
