...
* `kie` is the client resource name. If you want to use client roles with a different client resource name, replace `kie` with the client resource name, for example, `<resource>customClient</resource>`.
+
[NOTE]
====
