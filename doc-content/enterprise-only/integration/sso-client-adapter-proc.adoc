...
* `resource` is the name for the client that you created. If you want to use client roles, use the client resource name that you used when configuring the client in RH-SSO.
...
----
+
. Optional: The Elytron subsystem provides a built-in policy provider based on JACC specification. To enable the JACC manually in the `standalone.xml` or in the file where Elytron is installed, do any of the following tasks:
