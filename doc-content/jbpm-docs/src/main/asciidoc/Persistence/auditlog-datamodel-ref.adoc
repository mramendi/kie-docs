...
The `VariableInstanceLog` table contains information about changes in variable instances. By default, the {PROCESS_ENGINE} generates log entries after a variable changes its value. It is also possible to log entries before the changes.
