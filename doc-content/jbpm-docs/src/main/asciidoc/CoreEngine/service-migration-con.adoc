...
When migrating a process, the process instance migration service also automatically migrates all its subprocesses, subprocesses of its subprocesses, and so on. If you attempt to migrate a subprocess withput migrating the parent process, the migration fails.
