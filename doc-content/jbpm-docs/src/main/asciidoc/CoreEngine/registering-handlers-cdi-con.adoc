...
Package implementations of these interfaces as a bean archive by including `beans.xml` in the `META-INF` subdirectory. Place the bean archive on the application class path, for example, in `WEB-INF/lib` for a web application. The CDI-based `RuntimeManager` discovers the packages and registers the work item handlers and event listeners in every `KieSession` that is creates or loads from the data store.
