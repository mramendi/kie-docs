...
You can configure {KIE_SERVER} to emit Kafka messages about every event when a process, case, or task is completed. {KIE_SERVER} sends the messages when it commits transactions.
