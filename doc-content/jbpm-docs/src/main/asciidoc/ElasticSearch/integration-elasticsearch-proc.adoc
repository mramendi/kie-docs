...
You can configure the {KIE_SERVER} to send information to ElasticSearch about every event when a process, case, or task is completed. The {KIE_SERVER} sends the information when it commits transactions.
...
* `org.jbpm.integration.elasticsearch.url`: The URL of the ElasticSearch server. The default value is `http://localhost:9200`.
* `org.jbpm.integration.elasticsearch.index`: The ElasticSearch index to use for the information.
* `org.jbpm.integration.elasticsearch.date_format`: The timestamp format for the information. The default value is `yyyy-MM-dd'T'HH:mm:ss.SSSZ` .
* `org.jbpm.integration.elasticsearch.user`: The user name for authenticating to the ElasticSearch server.
* `org.jbpm.integration.elasticsearch.password`: The password for authenticating to the ElasticSearch server.
