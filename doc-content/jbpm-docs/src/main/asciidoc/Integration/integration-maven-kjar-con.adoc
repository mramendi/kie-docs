...
The following sample shows a custom `kie-deployment-descriptor.xml` file that configures the runtime engine. This example includes a single work item handler. You can also use the `kie-deployment-descriptor.xml` file to configure other options.
