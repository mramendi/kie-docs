...
* The following resources are available on the OpenShift cluster:
** For an authoring environment, 8 gigabytes of memory and 4 virtual CPU cores for the the {CENTRAL} pod. In a high-availability deployment, these resources are required for each replica and two replicas are created by default.
** For each {KIE_SERVER} pod, 1 gigabyte of memory and 1 virtual CPU core. If a {KIE_SERVER} pod is replicated, these resources are required for each replica. In a high-availability authoring environment, two replicas of the {KIE_SERVER} pod are created by default.
** In a high-availability authoring deployment, for the Red Hat Data Grid pod, 2 gigabytes of memory and 1 CPU core for each replica. Two replicas are created by default.
** For other pods, the default resource limits configured on your cluster.
