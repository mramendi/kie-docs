...
** Your OpenShift environment must support persistent volumes with `ReadWriteMany` mode. For information about access mode support in OpenShift Online volume plug-ins, see https://access.redhat.com/documentation/en-us/openshift_container_platform/3.11/html-single/architecture/#pv-access-modes[Access Modes].
+
[IMPORTANT]
====
...
include::{enterprise-dir}/openshift/rhdm72-authoring-ref.adoc[leveloffset=+2]
include::{enterprise-dir}/openshift/rhdm72-authoring-ha-ref.adoc[leveloffset=+2]
include::{enterprise-dir}/openshift/rhdm72-kieserver-ref.adoc[leveloffset=+2]
