[id='assembly_openshift-authoring-managed]

include::_artifacts/document-attributes.adoc[]

= Deploying a {PRODUCT} {PRODUCT_VERSION} authoring or managed server environment on {OPENSHIFT}
// Context attribute is assembly specific and enables module reuse between assemblies.
:doctype: book
:imagesdir: _images
:context: openshift-authoring-managed
include::_artifacts/author-group.adoc[]

// Purpose statement for the assembly
As a system engineer, you can deploy a {PRODUCT} authoring or managed environment on {OPENSHIFT} to provide a platform for developing or running {ASSETS} and other business assets.

.Prerequisites

* At least four gigabytes of memory are available in the OpenShift environment.
* The OpenShift project for the deployment is created.
* You have logged in to the project using the OpenShift web console and using the `oc` command.
* Dynamic persistent volume (PV) provisioning is enabled. Alternatively, enough persistent volumes are available. By default, the following sizes are required:
** The replicated set of {KIE_SERVER} pods requires one 1Gi PV for the database. This requirement does not apply if you use an external database server.
** {CENTRAL} requires one 1Gi PV.
* If you intend to use the Authoring High Availability template, the image streams for Red Hat AMQ version 7.0 or later are available in your OpenShift environment.
* If you intend to use the Authoring High Availability template, which scales the {CENTRAL} pod, your OpenShift environment supports persistent volumes with ReadWriteMany mode.
+
IMPORTANT: ReadWriteMany mode is not supported on OpenShift Online and OpenShift Dedicated.


include::{enterprise-dir}/openshift/ba-openshift-overview-con.adoc[leveloffset=+1]
include::{enterprise-dir}/openshift/dm-openshift-prepare-con.adoc[leveloffset=+1]
include::{enterprise-dir}/openshift/imagestreams-file-install-proc.adoc[leveloffset=+2]
include::{enterprise-dir}/openshift/secrets-kie-create-proc.adoc[leveloffset=+2]
include::{enterprise-dir}/openshift/secrets-central-create-proc.adoc[leveloffset=+2]
include::{enterprise-dir}/openshift/environment-authoring-managed-con.adoc[leveloffset=+1]
include::{enterprise-dir}/openshift/environment-authoring-single-proc.adoc[leveloffset=+2]
include::{enterprise-dir}/openshift/environment-authoring-ha-proc.adoc[leveloffset=+2]
include::{enterprise-dir}/openshift/kieserver-additional-deploy-proc.adoc[leveloffset=+2]

// Versioning info
include::_artifacts/versioning-information.adoc[]
