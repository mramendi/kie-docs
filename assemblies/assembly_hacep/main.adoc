...
Standard deployment of {PRODUCT} on {OPENSHIFT}, as described in {URL_DEPLOYING_OPENSHIFT_OPERATOR}[{DEPLOYING_OPENSHIFT_OPERATOR}], supports only stateless processing and therefore is not suitable for high available event-driven decisioning. You must create a custom implementation using the provided reference implementation.
