...
and other business assets. You can deploy any number of managed and immutable {KIE_SERVER} instances and control them using the same instance of {CENTRAL} Monitoring. You can add and remove the instances as necessary; {CENTRAL} Monitoring establishes a connection to them automatically.
