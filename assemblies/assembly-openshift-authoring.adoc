...
* The following resources are available on the OpenShift cluster. Depending on the load, higher resource allocation might be necessary for acceptable performance.
** For the {CENTRAL} pod, 4 gigabytes of memory and 2 virtual CPU cores. In a high-availability deployment, these resources are required for each replica and two replicas are created by default.
** For the {KIE_SERVER} pod, 2 gigabytes of memory and 1 virtual CPU core. In a high-availability deployment, these resources are required for each replica and two replicas are created by default.
** In a high-availability deployment, additional resources according to the configured defaults are required for the MySQL, Red Hat AMQ, and Red Hat Data Grid pods.
