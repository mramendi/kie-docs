...
** For the {CENTRAL} pod, 8 gigabytes of memory and 4 virtual CPU cores. In a high-availability deployment, these resources are required for each replica and two replicas are created by default.
** For the {KIE_SERVER} pod, 1 gigabyte of memory and 1 virtual CPU core. In a high-availability deployment, these resources are required for each replica and two replicas are created by default.
** In a high-availability deployment, for the Red Hat AMQ pod and the MySQL pod, the default resource limits configured on your cluster.
** In a high-availability deployment, for the Red Hat Data Grid pod, 2 gigabytes of memory and 1 CPU core for each replica. Two replicas are created by default.
